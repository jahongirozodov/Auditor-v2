...
import { deleteReport, regenerateReportSummary, reportApproval } from "@/lib/actions/reports";
import { canActAt, reportCurrentOf } from "@/lib/approval";
...
export function ReportsScreen({ reports, audits, usersById, role, currentUserId }: Props) {
  ...
  function download(r: Report) {
    ...
    window.open(`/print/reports/${r.id}`, "_blank", "noopener");
  }

  function regenerate(r: Report) {
    startTransition(async () => {
      const res = await regenerateReportSummary(r.id);
      toast(res.ok ? t("aiDone") : t("aiDegraded"), res.ok ? "success" : "warning");
      router.refresh();
    });
  }

  function approval(r: Report, action: "submit" | "approve" | "return") {
    let comment: string | undefined;
    if (action === "return") {
      const c = prompt(t("returnPrompt"));
      if (c === null) return;
      if (!c.trim()) return toast(t("commentRequired"), "danger");
      comment = c;
    }
    startTransition(async () => {
      const res = await reportApproval({ reportId: r.id, action, comment });
      if (res.ok) {
        toast(t(`${action}Done`), action === "return" ? "warning" : "success");
        router.refresh();
      } else {
      ...
      {filtered.length === 0 ? (
        ...
        >
          {filtered.map((r) => {
            ...
            const cur = reportCurrentOf(r.status, r.approvalStage);
            const isAuthor = r.author === currentUserId;
            const canSubmit =
              (cur === "new" || cur === "returned") && (isAuthor || canActAt(role, "group_lead"));
            const canActOnReview =
              (cur === "group_lead" || cur === "head" || cur === "dept") && canActAt(role, cur);
            return (
              <div key={r.id} className="card card--hover">
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      flexShrink: 0,
                      background:
                        "linear-gradient(180deg, var(--bg-surface) 0%, var(--brand-soft) 100%)",
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    {author && <Avatar initials={author.avatar} name={author.name} />}
                    <div>
