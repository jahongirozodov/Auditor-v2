...
import { generateReport, REPORT_TYPES, REPORT_FORMATS } from "@/lib/actions/reports";
    ...
    >
      <div style={{ display: "grid", gap: 14 }}>
        <Field label={t("fieldTitle")} htmlFor="rg-title">
          <input id="rg-title" className="input" value={title}
            onChange={(e) => setTitle(e.target.value)} autoFocus
            placeholder={t("fieldTitlePlaceholder")} />
        </Field>
        <Field label={t("fieldAudit")} htmlFor="rg-audit">
          <select id="rg-audit" className="select" value={auditId}
        ...
        <Field label={t("fieldType")} htmlFor="rg-type">
          <select id="rg-type" className="select" value={type}
