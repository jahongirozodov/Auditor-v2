...
export function UsersScreen({ users: initial, kpi, canEdit }: UsersScreenProps) {
  ...
  const kpiMap = useMemo(
    () => Object.fromEntries(kpi.map((k) => [k.user, k])),
    [kpi],
  );
  ...
  function openEdit(u: AdminUserView) { setEditing(u); setModal(true); }

  function lock(u: AdminUserView) {
    startTransition(async () => {
      const res = await toggleUserLock(u.id);
      if (res.ok) {
        toast(res.disabled ? t("locked", { name: u.name }) : t("unlocked", { name: u.name }), "warning");
        router.refresh();
      }
    });
  }
  ...
  function remove(u: AdminUserView) {
    ...
    startTransition(async () => {
      ...
      if (res.ok) { toast(t("deleted", { name: u.name }), "danger"); router.refresh(); }
  ...
  return (
    <div className="route-anim">
      ...
      <div className="grid" style={{ gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 16 }}>
        {roleCounts.map((r) => (
          <div key={r.code} className="card card__pad-sm"
            style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <div className="cell-sub"
              style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700 }}>
              {r.label.split(" ")[0]}
            </div>
      ...
      <Panel flush>
        <div className="tbl-scroll">
          <table className="tbl">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={8} style={{ textAlign: "center", padding: 24, color: "var(--text-tertiary)" }}>{t("empty")}</td></tr>
              ) : filtered.map((u) => {
                const k = kpiMap[u.id];
                return (
                  <tr key={u.id}>
                    <td>
                      <div className="cell-title">
                        ...
                        <div>
                          <div>{u.name}</div>
                          <div className="cell-sub">@{u.id} · {u.title}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      {u.disabled
                        ? <span className="tag tag--danger"><span className="dot" style={{ width: 6, height: 6 }} />Bloklangan</span>
                        : <span className="tag tag--success"><span className="dot" style={{ width: 6, height: 6 }} />Faol</span>
                      }
                    </td>
                    <td className="cell-actions">
                      {canEdit && (
                        <div style={{ display: "inline-flex", gap: 4, alignItems: "center" }}>
                          <button className="btn btn--ghost btn--xs btn--icon" title={t("edit")}
                            onClick={() => openEdit(u)} disabled={pending}>
                            <Edit3 size={13} />
                          </button>
                          <button className="btn btn--ghost btn--xs btn--icon"
                            title={u.disabled ? t("unlock") : t("lock")}
                            onClick={() => lock(u)} disabled={pending}>
                            {u.disabled ? <LockOpen size={13} /> : <Lock size={13} />}
                          </button>
                          <button className="btn btn--ghost btn--xs btn--icon btn--danger-hover"
                            title={t("delete")} onClick={() => remove(u)} disabled={pending}>
                            <Trash2 size={13} />
                          </button>
                        </div>
                      )}
