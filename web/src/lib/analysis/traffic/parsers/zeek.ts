  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (totalBytes > 10_000_000 && origH && respH) {
        largeTx.push({ srcIp: origH, dstIp: respH, bytes: totalBytes, port: isNaN(respP) ? "" : String(respP) });
      }
    }
    ...
    return {
      ...
      scanDate: isFinite(earliestTs) ? new Date(earliestTs * 1000).toISOString().slice(0, 10) : undefined,
    };
  } catch {
