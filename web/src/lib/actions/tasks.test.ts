...
const h = vi.hoisted(() => ({
  ...
  audit: { status: "in_progress", members: [{ userId: "u6" }] } as {
    status: string;
...
beforeEach(() => {
  ...
  h.audit = { status: "in_progress", members: [{ userId: "u6" }] };
...
describe("createTask", () => {
  it("creates the next code, assigns, and revalidates", async () => {
  ...
  it("rejects creation outside a work phase", async () => {
    h.audit = { status: "planning", members: [{ userId: "u6" }] };
...
describe("taskTransition KPI emission", () => {
  ...
  it("emits task_overdue (not task_on_time) when completed past due date", async () => {
    h.task = { id: "T-1", status: "review", assigneeId: "u1", auditId: "AUD-1", due: "2020-01-01" };
    await taskTransition({ taskId: "T-1", action: "approve" });
    const codes = mockPrisma.kpiEvent.create.mock.calls.map(
      (c: [{ data: { ruleCode: string } }]) => c[0].data.ruleCode,
    );
